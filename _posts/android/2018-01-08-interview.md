...
static可以修饰：
1. 静态导入：就是把一个静态变量或者静态方法一次性导入，导入后可以直接使用该变量或者方法，而不用写对象名
2. 静态变量：静态变量属于类，在内存中只有一个实例。当静态变量所在的类被加载的时候，就会为该静态变量分配内存空间，该变量就可以被使用。
3. 静态方法：static方法是类的方法，不需要创建对象就可以使用。**static方法中不能使用this和super关键字，不能调用非static方法，只能访问所属类的静态变量和静态方法。因为当static方法被调用的时候，这个类的对象可能还没有创建，即使已创建了，也不能确认调用哪个对象的方法。**
4. 静态代码块：静态代码块就是用static修饰的用{}括起来的代码段，它的主要目的就是对静态属性进行初始化。
5. 静态内部类：被static修饰的内部类，它可以不依赖于外部类实例对象而被实例化，而通常的内部类需要在外部类实例化后才能实例化。静态内部类不能与外部类有相同的名字，不能访问外部类的普通成员变量，只能访问内部类中的静态成员和静态方法（包括私有类型）。
